...
router.post('/login', async (req, res) => {
  try {
    ...
    const [users] = await promisePool.query(
      'SELECT * FROM gas_admin_users WHERE username = ? AND is_active = ?',
      [username, true]
    );
...
router.get('/auth/me', async (req, res) => {
  try {
    ...
    const decoded = jwt.verify(token, process.env.JWT_SECRET || 'gas_admin_secret_key');
    
    const [users] = await promisePool.query(
      'SELECT id, username, full_name, role, email, created_at FROM gas_admin_users WHERE id = ? AND is_active = ?',
      [decoded.id, true]
    );
...
router.get('/dashboard/stats', async (req, res) => {
  try {
    ...
    const [[{ pendingPayments }]] = await promisePool.query(
      "SELECT COALESCE(SUM(total_amount), 0) as pendingPayments FROM gas_bills WHERE status != 'paid'"
    );
    
    // Recent applications
    const [recentApplications] = await promisePool.query(
      `SELECT application_number, applicant_name as full_name, application_type, connection_type as gas_type, status, submission_date as submitted_at
    ...
    const [recentComplaints] = await promisePool.query(
      `SELECT c.complaint_number, gc.full_name as contact_name, c.complaint_type as complaint_category, c.priority as urgency, c.status, c.submitted_at as created_at
       FROM gas_complaints c
       JOIN gas_consumers gc ON c.consumer_id = gc.id
