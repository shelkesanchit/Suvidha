...
router.post('/send', async (req, res) => {
  ...
  try {
    ...
    await transporter.sendMail({
      from: `"SUVIDHA Water Services" <${process.env.EMAIL_USER}>`,
      to: email,
      subject: 'OTP for Water Application Submission – SUVIDHA Smart Utility Services',
      html: `
        <div style="font-family:Arial,sans-serif;max-width:520px;margin:0 auto;padding:24px;border:1px solid #e0e0e0;border-radius:12px;background:#fff">
          <div style="text-align:center;margin-bottom:20px">
            <div style="background:#0288d1;color:#fff;padding:12px 24px;border-radius:8px;display:inline-block">
              <h2 style="margin:0;font-size:22px;letter-spacing:1px">SUVIDHA</h2>
              <p style="margin:2px 0 0;font-size:11px;opacity:0.85">Smart Utility Kiosk - Water Services</p>
            </div>
          </div>
          <h3 style="color:#0288d1;margin-bottom:8px">Email Verification Required</h3>
          <p style="color:#555;font-size:14px;margin-bottom:4px">To complete your water application submission, please enter the OTP below:</p>
          <div style="background:#e1f5fe;border:2px dashed #0288d1;border-radius:10px;padding:28px;text-align:center;margin:20px 0">
            <p style="color:#888;font-size:12px;margin:0 0 8px">Your One-Time Password</p>
            <span style="font-size:42px;font-weight:bold;letter-spacing:10px;color:#0288d1">${otp}</span>
          </div>
    ...
    console.error('Water OTP email send error:', err);
...
// POST /api/water/otp/send-receipt
...
router.post('/send-receipt', async (req, res) => {
  ...
  const excludeKeys = ['password', 'otp', 'token', 'documents', 'agreed_to_terms', 'include_sewerage'];
  ...
  try {
    ...
    await transporter.sendMail({
      from: `"SUVIDHA Water Services" <${process.env.EMAIL_USER}>`,
      to: email,
      subject: `Application Receipt: ${application_number} – SUVIDHA Water Services`,
      html: `
        <div style="font-family:Arial,sans-serif;max-width:620px;margin:0 auto;padding:0;border:1px solid #e0e0e0;border-radius:12px;overflow:hidden">
          <!-- Header -->
          <div style="background:linear-gradient(135deg,#0288d1,#039be5);color:#fff;padding:28px 32px">
            <h1 style="margin:0;font-size:24px;letter-spacing:1px">SUVIDHA</h1>
            <p style="margin:4px 0 0;font-size:13px;opacity:0.85">Smart Utility Kiosk - Water Services – Government of India</p>
          </div>
          <!-- Title -->
          <div style="padding:24px 32px;background:#e1f5fe;border-bottom:1px solid #b3e5fc">
            <h2 style="margin:0 0 4px;color:#0288d1;font-size:18px">Application Receipt</h2>
          ...
          <div style="padding:24px 32px;background:#fff">
            <table style="width:100%;border-collapse:collapse;margin-bottom:20px">
              <tr>
                <td style="padding:10px;background:#e1f5fe;border-radius:8px;text-align:center" colspan="2">
                  <p style="margin:0;color:#555;font-size:12px;text-transform:uppercase;letter-spacing:1px">Application Number</p>
                  <p style="margin:4px 0 0;color:#0288d1;font-size:28px;font-weight:bold;letter-spacing:3px">${application_number}</p>
                </td>
              </tr>
            </table>
          ...
          <div style="padding:0 32px 24px;background:#fff">
            <h3 style="color:#0288d1;font-size:15px;margin-bottom:12px;border-bottom:2px solid #b3e5fc;padding-bottom:8px">Application Details</h3>
            <table style="width:100%;border-collapse:collapse">
              ${detailRows}
            </table>
          </div>` : ''}
          <!-- Next Steps -->
          <div style="padding:20px 32px;background:#e1f5fe;border-top:1px solid #b3e5fc">
          ...
          <!-- Footer -->
          <div style="padding:16px 32px;background:#0288d1;text-align:center">
    ...
    console.error('Water receipt email error:', err);
