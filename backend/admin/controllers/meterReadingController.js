...
// Get all customers from electricity_customers table
const getAllCustomers = async (req, res) => {
  try {
    const { state, city, area, pincode } = req.query;

    let query = `
      SELECT c.id, c.full_name AS name, c.mobile, c.email,
             c.state, c.city, c.area, c.plot_number AS plotNumber,
             c.pincode, c.meter_number AS meterNumber,
             c.connection_type AS connectionType, c.connection_status,
             mr.current_reading AS previousReading,
             mr.reading_date AS lastReadingDate
      FROM electricity_customers c
      LEFT JOIN (
        SELECT customer_id, current_reading, reading_date
        FROM electricity_meter_readings
        WHERE id IN (
          SELECT MAX(id) FROM electricity_meter_readings GROUP BY customer_id
        )
      ) mr ON c.id = mr.customer_id
    ...
    const formatted = customers.map(c => ({
      ...
      connectionType: c.connectionType
        ? c.connectionType.charAt(0).toUpperCase() + c.connectionType.slice(1)
        : 'Domestic',
    }));
...
// Returns { consumptionCharges, fixedCharges, taxAmount, totalAmount, category }
async function calculateBill(consumption, customerId) {
  try {
    // Get customer details for tariff lookup
    const [customers] = await pool.query(
      'SELECT state, city, connection_type FROM electricity_customers WHERE id = ?',
      [customerId]
    );
    ...
    const { state, city, connection_type } = customers[0];
    const category = connection_type || 'domestic';
