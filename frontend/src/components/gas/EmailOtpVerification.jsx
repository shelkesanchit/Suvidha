...
/**
 * EmailOtpVerification — Gas department email OTP verification dialog.
 *
 * Props:
 *  open           {boolean}
 *  onClose        {function}
...
const EmailOtpVerification = ({ open, onClose, onVerified, initialEmail = '', title = 'Verify Your Email' }) => {
  ...
  const handleSendOtp = async () => {
    ...
    setLoading(true);
    try {
      await api.post('/gas/otp/send', { email });
      setStep('otp');
      setCountdown(60);
  ...
  const handleVerifyOtp = async () => {
    ...
    setLoading(true);
    try {
      await api.post('/gas/otp/verify', { email, otp });
      setStep('verified');
  ...
  return (
    ...
    >
      <Box sx={{ background: 'linear-gradient(135deg, #e65100, #f57c00)', px: 3, py: 2.5 }}>
        ...
        {step === 'email' && (
          <Box>
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 2 }}>
              <EmailIcon sx={{ color: '#e65100', fontSize: 28 }} />
          ...
          <DialogActions sx={{ px: 3, py: 2 }}>
            ...
            {step === 'email' && (
              <Button
                ...
                sx={{ background: '#e65100', '&:hover': { background: '#bf360c' } }}
              >
                Send OTP
              </Button>
            )}
